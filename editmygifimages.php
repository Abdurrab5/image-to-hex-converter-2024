    ...
    if ($_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($file_type, $allowed_types)) {
            ...
            if (move_uploaded_file($file_tmp, $target_file)) {
                ...
                $query = "UPDATE gif_images SET file_name = ?, file_path = ? WHERE id = ? AND user_id = ?";
            ...
            echo "<p class='error'>Invalid file type. Allowed types: GIF.</p>";
        }
    } else {
...
if (!empty($_GET['delete_id'])) {
    ...
    $sql = "SELECT file_path FROM gif_images WHERE id = ? AND user_id = ?";
    $stmt = $conn->prepare($sql);
    ...
    if ($result->num_rows > 0) {
        ...
        $delete_sql = "DELETE FROM gif_images WHERE id = ? AND user_id = ?";
...
<main>
    <div class="regcontainer">
        ...
        <form method="POST" enctype="multipart/form-data" id="signupform">
            ...
            <!-- File upload input -->
            <div class="inputBox">
                <input type="file" name="image" id="imageInput" required accept="image/gif">
            </div>
